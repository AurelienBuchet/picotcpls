...
def run_s3_c3(net):
    net['s3'].cmd('iperf3 -s > s3.out &')
    net['c3'].cmd('iperf3 -c 192.168.7.100 -u -n 10G -b 90M > c3.out ')
...
def run():
    ...
    """
    net['c1'].cmd('bash achange_cc.sh vegas')
    net['s1'].cmd('bash achange_cc.sh vegas')
    net['c2'].cmd('bash achange_cc.sh cubic')
    net['s2'].cmd('bash achange_cc.sh cubic')
    time.sleep(10)
    pid1 = os.fork()
    if(pid1 == 0):
        run_s1(net)

...
static int prepare_stream_attach(uint8_t *buf, size_t *len, uint32_t stream_id, uint32_t transportid, uint32_t offset);
static int prepare_failover(uint8_t *buf, size_t *len, uint32_t peer_transportid, uint32_t streamid, uint32_t seq);

static void refill_con_tokens(connect_info_t *con);
...
int tcpls_handshake(ptls_t *tls, ptls_handshake_properties_t *properties) {
  ...
  if (!ret) {
    ...
    if (con->this_transportid != 0) {
      uint8_t input[4];
      ptls_buffer_init(&sendbuf, "", 0);
      uint32_t this_transportid = htonl(con->this_transportid);
      memcpy(input, &this_transportid, 4);
...
int tcpls_accept(tcpls_t *tcpls, int socket, uint8_t *cookie, uint32_t transportid) {
  ...
  if (cookie) {
    ...
    int ret;
    if (!con)
      this_transportid = htonl(newconn.this_transportid);
    else
      this_transportid = htonl(con->this_transportid);
...
int tcpls_streams_attach(ptls_t *tls, streamid_t streamid, int sendnow) {
  ...
  for (int i = 0; i < tcpls->streams->size; i++) {
    ...
    if (stream_to_attach->need_sending_attach_event) {
      ...
      prepare_stream_attach(input, &input_len, stream_to_attach->streamid, con->this_transportid, stream_to_attach->offset);
      if (input_len > sizeof(input)) {
          return -1;
      }
      stream_send_control_message(tls, stream_to_attach->streamid,
          sendbuf_to_use, ctx_to_use, input, STREAM_ATTACH, input_len);
...
int tcpls_send(ptls_t *tls, streamid_t streamid, const void *input, size_t nbytes) {
  ...
  if (!tcpls->streams->size && ((tcpls->tls->is_server && tcpls->next_stream_id
    ...
    /** send the stream id to the peer */
    ret = prepare_stream_attach(message, &message_len, stream->streamid, con->this_transportid, tcpls->nbr_of_our_streams_attached);
    if (ret || message_len > sizeof(message)) {
      return -1;
    }
  ...
  if(con->is_limited){
    refill_con_tokens(con);
    if(con->tokens < nbytes){
      return TCPLS_CON_LIMIT_REACHED;
    }
    con->tokens -= nbytes;
  }
...
int tcpls_send_tcpoption(tcpls_t *tcpls, int transportid, tcpls_enum_t type, int sendnow)
{
  ...
  if (option->is_varlen) {
    ...
    uint32_t option_data_len = htonl(option->data->len);
    memcpy(input, &option_data_len, 4);
...
int tcpls_ping_tcp(tcpls_t *tcpls, int transportid){
  ...
  if (!con)
    return PTLS_ERROR_CONN_NOT_FOUND;
  struct tcp_info our_infos;
  uint32_t info_length = sizeof(struct tcp_info);
  ...
  size_t message_len = sizeof(message);
  int offset = 0;
  if(message_len >= 4*sizeof(uint32_t)){
    memcpy(message + offset, &transportid, 4);
    offset += 4;
 ...
 */
 int tcpls_limit_con(tcpls_t *tcpls, int transportid, uint64_t rate){
  ...
  if (!con)
    return PTLS_ERROR_CONN_NOT_FOUND;
  con->is_limited = 1;
  con->tokens = PTLS_MAX_ENCRYPTED_RECORD_SIZE;
  con->max_tokens = 4*PTLS_MAX_ENCRYPTED_RECORD_SIZE;
  con->token_rate = rate;
  gettimeofday(&(con->last_refill), NULL);
  return 0;
 }
 ...
 int tcpls_limit_peer_con(tcpls_t *tcpls, int transportid, uint64_t rate){
  ...
  if (!con)
    return PTLS_ERROR_CONN_NOT_FOUND;
  uint8_t message[sizeof(uint32_t) + sizeof(uint64_t)];
  int message_len = sizeof(message);
  int offset = 0;
  transportid = htonl(transportid);
  
  if (message_len >= sizeof(uint32_t) + sizeof(uint64_t)) {
      memcpy(message + offset, &transportid, 4);
      offset += 4;
      memcpy(message + offset, &rate, 8);
  } else {
    return PTLS_ERROR_NO_MEMORY;
  }
  stream_send_control_message(tcpls->tls, 0, tcpls->sendbuf, tcpls->tls->traffic_protection.enc.aead,
    message, FLOW_CONTROL, message_len);
  if (do_send(tcpls, 0, con) <= 0) {
    //XXX
    fprintf(stderr, "Unimplemented\n");
  }
...
/**
 * Prepares a STREAM_ATTACH message content in buf, and sets len to the written size
 */
static int prepare_stream_attach(uint8_t *buf, size_t *len, uint32_t stream_id, uint32_t transportid, uint32_t offset) {
  int ret = 0;
  size_t consumed = 0;
  ...
  if (*len >= 3 * sizeof(uint32_t)) {
      memcpy(buf + consumed, &stream_id, 4);
      consumed += 4;
      memcpy(buf + consumed, &transportid, 4);
      consumed += 4;
      memcpy(buf + consumed, &offset, 4);
      consumed += 4;
  } else {
...
/**
 * Prepares a FAILOVER message content in buf, and sets len to the written size
 */
static int prepare_failover(uint8_t *buf, size_t *len, uint32_t peer_transportid, uint32_t streamid, uint32_t seq) {
  int ret = 0;
  size_t consumed = 0;
  ...
  seq = htonl(seq);
  if (*len >= 3 * sizeof(uint32_t)) {
    memcpy(buf + consumed, &peer_transportid, 4);
    consumed += 4;
    memcpy(buf + consumed, &streamid, 4);
    consumed += 4;
    memcpy(buf + consumed, &seq, 4);
    consumed += 4;
  } else {
    ret = PTLS_ERROR_NO_MEMORY;
  }
  ...
  if (!tcpls->tls->is_server) {
    ...
    for (int i = 0; i < tcpls->streams->size; i++) {
      ...
      if (stream_failed->transportid == con->this_transportid) {
        ...
        if (seq == 1)
          seq--;
        prepare_failover(input, &input_len, con->peer_transportid, stream_failed->transportid, seq);
        if (found)
          stream_send_control_message(tcpls->tls, stream_to_use->streamid,
              stream_to_use->sendbuf, stream_to_use->aead_enc, input, FAILOVER, input_len);
        else {
          tcpls->sending_stream = NULL;
          stream_send_control_message(tcpls->tls, 0, tcpls->sendbuf,
              tcpls->tls->traffic_protection.enc.aead, input, FAILOVER, input_len);
        }
        stream_failed->stream_usable = 0;
...
int handle_tcpls_control(ptls_t *ptls, tcpls_enum_t type,
  ...
  switch (type) {
    ...
    case TRANSPORT_NEW:
      {
        ...
        uint32_t our_transportid = ntohl(*(uint32_t*) &input[4]);
        connect_info_t *con;
    ...
    case STREAM_CLOSE:
      {
        ...
        streamid_t streamid = ntohl(*(streamid_t *)input);
    ...
    case STREAM_ATTACH:
      {
        streamid_t streamid = ntohl(*(streamid_t *) input);
        uint32_t peer_transportid = ntohl(*(uint32_t*) &input[4]);
        uint32_t offset = ntohl(*(uint32_t*) (&input[8]));
        connect_info_t *con;
        int found = 0;
    ...
    case DATA_ACK:
      {
        uint32_t streamid = ntohl(*(uint32_t *) input);
        uint32_t seqnum = ntohl(*(uint32_t *) &input[4]);
    ...
    case FAILOVER:
      {
        uint32_t transportid = ntohl(*(uint32_t*)input);
        uint32_t streamid = ntohl(*(uint32_t *)&input[4]);
        uint32_t stream_seq = ntohl(*(uint32_t *)&input[8]);
        connect_info_t *con = connection_get(ptls->tcpls, ptls->tcpls->transportid_rcv);
        ...
        if (ptls->is_server) {
          ...
          if (seq == 1)
            seq--;
          prepare_failover(message, &message_len, con_failed->peer_transportid, streamid, seq);
          if (found) {
            stream_send_control_message(ptls, stream_to_use->streamid,
                stream_to_use->sendbuf, stream_to_use->aead_enc, message, FAILOVER, message_len);
          }
          else {
            ptls->tcpls->sending_stream = NULL;
            stream_send_control_message(ptls, 0, ptls->tcpls->sendbuf,
                ptls->traffic_protection.enc.aead, message, FAILOVER,
                                        message_len);
          }
    ...
    case FAILOVER_END:
      {
        uint32_t this_transportid = ntohl(*(uint32_t*)input);
        uint32_t streamid = ntohl(*(uint32_t*) &input[4]);
        ptls->tcpls->nbr_remaining_failover_end--;
    ...
    case PING_RTT:
      {
        ...
        if(ptls->ctx->rtt_event_cb){
          ptls->ctx->rtt_event_cb(ptls->tcpls, PING_RTT_RECEIVED, peer_timestamp, con->this_transportid);
        }
        uint8_t message[4 + sizeof(struct timeval)];
        peer_transportid = htonl(peer_transportid);
        size_t message_len = sizeof(message);
    ...
    {
      uint32_t peer_transportid = ntohl(*(uint32_t*) input);
      uint64_t limit_rate = *(uint64_t *) &input[4];
...
int handle_tcpls_data_record(ptls_t *tls, struct st_ptls_record_t *rec)
{
  ...
  if (tcpls->enable_multipath) {
    mpseq = ntohl(*(uint32_t *) &rec->fragment[rec->length-sizeof(mpseq)]);
...
int handle_tcpls_control_record(ptls_t *tls, struct st_ptls_record_t *rec)
{
  ...
  type = ntohl(*(uint32_t *) &rec->fragment[rec->length-sizeof(uint32_t)]);
  ...
  if (is_varlen(type)){
    ...
    if (type == CONTROL_VARLEN_BEGIN) {
      ...
      if (tcpls->enable_multipath) {
        mpseq = ntohl(*(uint32_t *) &rec->fragment[rec->length-sizeof(uint32_t)]);
        rec->length -= sizeof(mpseq);
        connect_info_t *con = connection_get(tcpls, tcpls->transportid_rcv);
        assert(con);
        ...
        for (int i = 0; i < tcpls->streams->size; i++) {
          ...
          if (stream_failed->orcon_transportid == con->this_transportid &&
            ...
            if (stream_failed->send_start == stream_failed->sendbuf->off) {
              char input[8];
              uint32_t peer_transportid = htonl(con_to_failover->peer_transportid);
              uint32_t stream_id = htonl(stream_failed->streamid);
              memcpy(input, &peer_transportid, 4);
              memcpy(input+4, &stream_id, 4);
...
/**
 * Refill the tokens of the given connection 
 */
static void refill_con_tokens(connect_info_t *con){
  struct timeval now;
  gettimeofday(&now, NULL);
  struct timeval diff = timediff(&now, &(con->last_refill));
  con->tokens = con->tokens + (diff.tv_sec * con->token_rate) + ((diff.tv_usec * con->token_rate) / 1000000);
  if(con->tokens > con->max_tokens){
    con->tokens = con->max_tokens;
  }
  con->last_refill = now;
}
...
static int send_ack_if_needed__do(tcpls_t *tcpls, tcpls_stream_t *stream) {
  ...
  uint8_t input[4+4];
  uint32_t streamid = htonl(stream->streamid);
  uint32_t seq = htonl(stream->last_seq_received);
  memcpy(input, &streamid, 4);
  memcpy(&input[4], &seq, 4);
  tcpls->sending_stream = stream;
 ...
 *
 * It allows the counter to start at 0 using the same key for all streams.
 * Prevent different streams to overlap their GCM counter
 * 
...
static int new_stream_derive_aead_context(ptls_t *tls, tcpls_stream_t *stream, int is_client_origin) {
  ...
  if ((ret = ptls_hkdf_expand_label(tls->cipher_suite->hash, key,
          tls->cipher_suite->aead->key_size, ptls_iovec_init(ctx_enc->secret,
            tls->cipher_suite->hash->digest_size), "key", ptls_iovec_init(NULL, 0),
          tls->ctx->hkdf_label_prefix__obsolete)) != 0)
      return -1;
  if ((ret = ptls_hkdf_expand_label(tls->cipher_suite->hash, iv,
          tls->cipher_suite->aead->iv_size, ptls_iovec_init(ctx_enc->secret,
            tls->cipher_suite->hash->digest_size), "iv", ptls_iovec_init(NULL, 0),
          tls->ctx->hkdf_label_prefix__obsolete)) != 0)
      return -1;
  /** Derive enc iv */
  ...
  if ((ret = ptls_hkdf_expand_label(tls->cipher_suite->hash, key,
      ...
      return -1;
  if ((ret = ptls_hkdf_expand_label(tls->cipher_suite->hash, iv,
          tls->cipher_suite->aead->iv_size, ptls_iovec_init(ctx_dec->secret,
            tls->cipher_suite->hash->digest_size), "iv", ptls_iovec_init(NULL, 0),
          tls->ctx->hkdf_label_prefix__obsolete)) != 0)
      return -1;
  /** Derive dec iv */
